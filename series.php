...
$countStmt = $pdo->prepare("SELECT COUNT(*) FROM content WHERE type = 'series' AND status = 'published'");
$countStmt->execute();
...
$stmt = $pdo->prepare("SELECT c.*, cat.name as category_name, g.name as genre_name 
                       ...
                       WHERE c.type = 'series' AND c.status = 'published' 
...
<section class="section">
    <div class="section-header">
        ...
        <span style="color: var(--text-muted);"><?php echo number_format($total); ?> مسلسل</span>
    </div>
    ...
    <div class="empty-state">
        ...
        <p>سيتم إضافة المسلسلات قريباً</p>
    </div>
    <?php else: ?>
